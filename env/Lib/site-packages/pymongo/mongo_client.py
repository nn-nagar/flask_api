...
class MongoClient(common.BaseObject):
    ...
    def _end_sessions(self, session_ids):
        ...
        try:
            ...
            with self._socket_for_reads(
                    ...
                    None) as (sock_info, subordinate_ok):
    ...
    @contextlib.contextmanager
    def _subordinateok_for_server(self, read_preference, server, session,
                            exhaust=False):
    ...
    def _run_operation_with_response(self, operation, unpack_res,
        ...
        def _cmd(session, server, sock_info, subordinate_ok):
            return server.run_operation_with_response(
                sock_info,
                operation,
                subordinate_ok,
        ...
        while True:
            try:
                ...
                if not server.description.retryable_reads_supported:
                    retryable = False
                with self._subordinateok_for_server(read_pref, server, session,
                    ...
                    if retrying and not retryable:
                        ...
                        raise last_error
                    return func(session, server, sock_info, subordinate_ok)
            except ServerSelectionTimeoutError:
                if retrying:
                    # The application may think the write was never attempted
                    # if we raise ServerSelectionTimeoutError on the retry
...
class _MongoClientErrorHandler(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        elif issubclass(exc_type, NotMainError):
            # As per the SDAM spec if:
            #   - the server sees a "not main" error, and
