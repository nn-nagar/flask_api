...
class ClientEncryption(object):
          ...
          - `kms_providers`: Map of KMS provider options. Two KMS providers
            ...
            differ by provider:
              ...
              - `local`: Map with "key" as a 96-byte array or string. "key"
                is the main key used to encrypt/decrypt data keys. This key
    ...
    def create_data_key(self, kms_provider, main_key=None,
                        key_alt_names=None):
        ...
        :Parameters:
          - `kms_provider`: The KMS provider to use. Supported values are
            "aws" and "local".
          - `main_key`: The `main_key` identifies a KMS-specific key used
            to encrypt the new data key. If the kmsProvider is "local" the
            `main_key` is not applicable and may be omitted.
            If the `kms_provider` is "aws", `main_key` is required and must
        ...
        with _wrap_encryption_errors():
            return self._encryption.create_data_key(
                kms_provider, main_key=main_key,
